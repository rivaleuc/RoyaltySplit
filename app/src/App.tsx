  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border"><div className="mx-auto flex h-16 max-w-5xl items-center gap-2.5 px-5">
        ...
        <div className="ml-4 hidden font-mono text-xs text-muted md:block"><b className="text-accent"><NumberTicker value={Number(toGen(stats.distributed_wei).toFixed(2))} decimalPlaces={2} /></b> GEN split · <b className="text-foreground"><NumberTicker value={stats.settled} /></b> settled</div>
        <Button size="sm" className="ml-auto" variant="outline" onClick={() => setOpen(!open)}><Plus className="h-4 w-4" /> Work</Button>
      ...
      <div className="mx-auto max-w-5xl px-5 pt-5">
        {open && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="overflow-hidden">
            <div className="mb-4 flex gap-2 rounded-xl border border-border bg-card/60 p-3"><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Work title (song, paper, repo…)" className="flex-1 rounded-md border border-border bg-background/70 px-3 py-2 text-sm outline-none focus:border-primary/50" /><Button size="sm" onClick={create} disabled={creating}>{creating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Split className="h-4 w-4" />} Create</Button></div>
          </motion.div>
        )}
        <div className="flex flex-wrap gap-2">{works.map((x) => <button key={x.id} onClick={() => setSel(x.id)} className={`max-w-[200px] truncate rounded-lg border px-3 py-1.5 text-xs ${sel === x.id ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted hover:text-foreground'}`}>{x.title}</button>)}</div>
      </div>

      {!w ? <div className="mx-auto max-w-5xl px-5 py-24 text-center text-sm text-muted">No works yet.</div> : (
        <main className="mx-auto grid max-w-5xl gap-7 px-5 py-7 md:grid-cols-[260px_1fr]">
          {/* donut */}
          <div className="flex flex-col items-center">
            ...
            <div className="mt-1 text-lg font-bold">{w.title}</div>
          </div>
          ...
          <div>
            ...
            {!settled && (
              <div className="mt-3 space-y-2">
                <div className="flex flex-wrap gap-2"><input value={ct.c} onChange={(e) => setCt({ ...ct, c: e.target.value })} placeholder="Contributor 0x…" className="w-40 rounded-md border border-border bg-background/70 px-3 py-2 text-sm outline-none focus:border-primary/50" /><input value={ct.e} onChange={(e) => setCt({ ...ct, e: e.target.value })} placeholder="What they contributed" className="min-w-0 flex-1 rounded-md border border-border bg-background/70 px-3 py-2 text-sm outline-none focus:border-primary/50" /><Button size="sm" variant="outline" disabled={busy === 'add'} onClick={() => addC(w)}><UserPlus className="h-4 w-4" /></Button></div>
                {w.contributions.length >= 1 && <div className="flex gap-2"><div className="relative flex-1"><input value={rev} onChange={(e) => setRev(e.target.value)} placeholder="Revenue to split" className="w-full rounded-md border border-border bg-background/70 px-3 py-2 pr-12 text-sm outline-none focus:border-primary/50" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-accent">GEN</span></div><Button size="sm" disabled={busy === w.id} onClick={() => settle(w)}>{busy === w.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Coins className="h-4 w-4" />} Settle &amp; split</Button></div>}
              </div>
            )}
          </div>
        </main>
      )}
      <footer className="border-t border-border"><div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-6 text-xs text-muted"><span>RoyaltySplit · consensus-weighted royalty splits</span><a href={EXPLORER} target="_blank" rel="noreferrer" className="hover:text-primary">{short(CONTRACT)} ↗</a></div></footer>
    </div>
  )
}
